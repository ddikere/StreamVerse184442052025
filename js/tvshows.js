...
async function fetchNewTVShows() {
  ...
  const res = await fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&first_air_date.gte=${lastMonth}&first_air_date.lte=${today}&sort_by=first_air_date.desc`);
...
function displayGenres(genres) {
  const genresContainer = document.getElementById('tvshow-genres');
  genresContainer.innerHTML = '';
  genres.forEach(async (genre) => {
    const genreTVShows = await fetchTVShowsByGenre(genre.id);
    ...
    displayList(genreTVShows, `genre-${genre.id}`);
  });
}
...
function sortTVShows() {
  const sortBy = document.getElementById('tvshows-sort').value;
  let sortedTVShows = [...tvShows];
  if (sortBy === 'first_air_date.desc') {
    sortedTVShows.sort((a, b) => new Date(b.first_air_date) - new Date(a.first_air_date));
  } else if (sortBy === 'name.asc') {
    sortedTVShows.sort((a, b) => a.name.localeCompare(b.name));
  }
  displayList(sortedTVShows, 'new-tvshows-list');
}

async function init() {
  tvShows = await fetchNewTVShows();
  displayList(tvShows, 'new-tvshows-list');
